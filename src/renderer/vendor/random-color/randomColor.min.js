!(function (r, e) {
					...
					(e = (function (r) {
						{
							if (i.length > 0) {
								...
								const n = (o[1] - o[0]) / i.length;
								let t = parseInt((e - o[0]) / n);
								!0 === i[t] ? (t = (t + 2) % i.length) : (i[t] = !0);
								const a = (o[0] + t * n) % 359;
								const u = (o[0] + (t + 1) * n) % 359;
	...
	function l(r) {
		for (const e in (r >= 334 && r <= 360 && (r -= 360), s)) {
			const n = s[e];
			if (n.hueRange && r >= n.hueRange[0] && r <= n.hueRange[1]) {
				return s[e];
			}
		}
	...
	function c(r) {
		...
		const n = r[1] || 1;
		const t = r[0] || 0;
		const a = (o = (9301 * o + 49297) % 233280) / 233280;
	...
	function g(r) {
		r = (r = r.replace(/^#/, "")).length === 3 ? r.replace(/(.)/g, "$1$1") : r;
		const e = parseInt(r.substr(0, 2), 16) / 255;
		const n = parseInt(r.substr(2, 2), 16) / 255;
		const t = parseInt(r.substr(4, 2), 16) / 255;
		const a = Math.max(e, n, t);
		const u = a - Math.min(e, n, t);
		const o = a ? u / a : 0;
		switch (a) {
			case e:

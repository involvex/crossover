...
const isRed = (r, g, b) => {
    return r >= RED_MIN && r <= RED_MAX && g <= GREEN_MAX && b <= BLUE_MAX;
};
...
const analyzeImageData = (imageData) => {
    ...
    if (redPixelPercentage >= ENEMY_THRESHOLD_PERCENTAGE) {
        ...
        set.rendererProperties({ '--reticle-fill-color': preferences.value('crosshair.color') }, windows.win);
    }
};
...
const startDetection = () => {
    ...
    detectionInterval = setInterval(() => {
        ...
        if (detectionMode === 'pixel') {
            ...
            windows.win.webContents.send('request_screen_capture', captureRect, primaryDisplay.id);
